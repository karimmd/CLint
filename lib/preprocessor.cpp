            ...
            if (!suppressionIDs.empty()) {
                if (_settings != nullptr) {
                    ...
                    for (std::size_t j = 0; j < suppressionIDs.size(); ++j) {
                        ...
                        if (!errmsg.empty()) {
                            writeError(filename, lineno, _errorLogger, "clintError", errmsg);
                        }
                    }
                }
...
void Preprocessor::missingInclude(const std::string &filename, unsigned int linenr, const std::string &header, HeaderTypes headerType)
{
    ...
    if (_errorLogger && _settings->checkConfiguration) {
        ...
        ErrorLogger::ErrorMessage errmsg(locationList, Severity::information,
                                         ...
                                         "Include file: <" + header + "> not found. Please note: Clint does not need standard library headers to get proper results." :
...
private:
    ...
    /** prefix that is used by clint to separate macro parameters. Always "__clint__" */
...
public:
    ...
    explicit PreprocessorMacro(const std::string &macro)
        : _macro(macro), _prefix("__clint__") {
