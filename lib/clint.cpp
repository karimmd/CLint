...
static const char Version[] = CLINT_VERSION_STRING;
static const char ExtraVersion[] = "";

static TimerResults S_timerResults;

CLint::CLint(ErrorLogger &errorLogger, bool useGlobalSuppressions)
...
const char * CLint::version()
{
    return Version;
}

const char * CLint::extraVersion()
{
    return ExtraVersion;
}

unsigned int CLint::check(const std::string &path)
{
    return processFile(path, "");
}

unsigned int CLint::check(const std::string &path, const std::string &content)
{
    return processFile(path, content);
}

void CLint::replaceAll(std::string& code, const std::string &from, const std::string &to)
{
    std::size_t pos = 0;
...
bool CLint::findError(std::string code, const char FileName[])
{
...
unsigned int CLint::processFile(const std::string& filename, const std::string& fileContent)
{
    exitcode = 0;
...
void CLint::internalError(const std::string &filename, const std::string &msg)
{
...
void CLint::analyseFile(std::istream &fin, const std::string &filename)
{
    // Preprocess file..
    Preprocessor preprocessor(&_settings, this);
    std::list<std::string> configurations;
...
//---------------------------------------------------------------------------
// CLint - A function that checks a specified file
//---------------------------------------------------------------------------

void CLint::checkFile(const std::string &code, const char FileName[])
{
...
Settings &CLint::settings()
{
    return _settings;
}

void CLint::tooManyConfigsError(const std::string &file, const std::size_t numberOfConfigurations)
{
    ...
    msg << "Too many #ifdef configurations - clint only checks " << _settings._maxConfigs;
...
void CLint::reportErr(const ErrorLogger::ErrorMessage &msg)
{
...
void CLint::reportOut(const std::string &outmsg)
{
    _errorLogger.reportOut(outmsg);
}

void CLint::reportProgress(const std::string &filename, const char stage[], const std::size_t value)
{
    _errorLogger.reportProgress(filename, stage, value);
}

void CLint::reportInfo(const ErrorLogger::ErrorMessage &msg)
{
...
void CLint::reportStatus(unsigned int /*fileindex*/, unsigned int /*filecount*/, std::size_t /*sizedone*/, std::size_t /*sizetotal*/)
{

}

void CLint::getErrorMessages()
{
    tooManyConfigs = true;
