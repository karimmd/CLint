...
void CheckOther::variableScopeError(const Token *tok, const std::string &varname)
{
    reportError(tok,
                ...
                "example where clint will write that the scope for 'i' can be reduced:\n"
                "void f(int x)\n"
                "{\n"
                "    int i = 0;\n"
                "    if (x) {\n"
...
void CheckOther::commaSeparatedReturnError(const Token *tok)
{
    reportError(tok,
                ...
                "        return a + 1,\n"
                "    b++;\n"
                "However it can be useful to use comma in macros. Clint does not warn when such a "
...
void CheckOther::checkDuplicateBranch()
{
    ...
    //   here is noise since the code is fine (see clint, abiword, ..)
