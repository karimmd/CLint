...
std::string ErrorLogger::ErrorMessage::getXMLHeader(int xml_version)
{
    ...
    if (xml_version == 2) {
        ...
        printer.OpenElement("clint", false);
        printer.PushAttribute("version", CLint::version());
...
std::string ErrorLogger::ErrorMessage::toString(bool verbose, const std::string &outputFormat) const
{
    ...
    // template is given. Reformat the output according to it
        ...
        // Support a few special characters to allow to specific formatting, see http://sourceforge.net/apps/phpbb/clint/viewtopic.php?f=4&t=494&sid=21715d362c0dbafd3791da4d9522f814
        // Substitution should be done first so messages from clint never get translated.
