...
bool CmdLineParser::ParseFromArgs(int argc, const char* const argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        // (Experimental) exception handling inside clint client
            ...
            CLintExecutor::setExceptionOutput(exceptionOutfilename);
        }
        ...
        // Enforce language (--language=, -x)
            ...
            if (argv[i][2]) {
                ...
                i++;
                if (i >= argc || argv[i][0] == '-') {
                    PrintMessage("clint: No language given to '-x' option.");
                    return false;
                }
                str = argv[i];
            }
            ...
            if (str == "c")
                ...
                PrintMessage("clint: Unknown language '" + str + "' enforced.");
                return false;
            }
        }
        ...
        // Filter errors
            ...
            if (std::strcmp(argv[i], "--exitcode-suppressions") == 0) {
                ...
                if (i >= argc || std::strncmp(argv[i], "-", 1) == 0 ||
                    ...
                    PrintMessage("clint: No filename specified for the '--exitcode-suppressions' option.");
                    return false;
                }
                filename = argv[i];
            }
            ...
            if (!f.is_open()) {
                PrintMessage("clint: Couldn't open the file: \"" + filename + "\".");
                return false;
            }
        ...
        // Filter errors
            ...
            std::ifstream f(filename.c_str());
            if (!f.is_open()) {
                std::string message("clint: Couldn't open the file: \"");
                message += filename;
                message += "\".";
                if (std::count(filename.begin(), filename.end(), ',') > 0 ||
                    ...
                    message += "\n    clint --suppressions-list=a.txt --suppressions-list=b.txt file.cpp";
                }
        ...
        // This is deprecated, see --suppressions-list above
            ...
            if (i >= argc) {
                PrintMessage("clint: No file specified for the '--suppressions' option.");
                return false;
            }
        ...
        // Output relative paths
            ...
            if (argv[i][argv[i][3]=='='?4:17] != 0) {
                ...
                PrintMessage("clint: No paths specified for the '" + std::string(argv[i]) + "' option.");
                return false;
            }
        }
        ...
        // Define the XML file version (and enable XML output)
            ...
            if (!(iss >> _settings->_xml_version)) {
                PrintMessage("clint: argument to '--xml-version' is not a number.");
                return false;
            }

            if (_settings->_xml_version < 0 || _settings->_xml_version > 2) {
                // We only have xml versions 1 and 2
                PrintMessage("clint: '--xml-version' can only be 1 or 2.");
                return false;
            }
        ...
        // --error-exitcode=1
            ...
            if (!(iss >> _settings->_exitCode)) {
                ...
                PrintMessage("clint: Argument must be an integer. Try something like '--error-exitcode=1'.");
                return false;
            }
        }
        ...
        // User define
            ...
            if (std::strcmp(argv[i], "-D") == 0) {
                ++i;
                if (i >= argc || argv[i][0] == '-') {
                    PrintMessage("clint: argument to '-D' is missing.");
                    return false;
                }
        ...
        // User undef
            ...
            if (std::strcmp(argv[i], "-U") == 0) {
                ++i;
                if (i >= argc || argv[i][0] == '-') {
                    PrintMessage("clint: argument to '-U' is missing.");
                    return false;
                }
        ...
        // Include paths
            ...
            if (std::strcmp(argv[i], "-I") == 0) {
                ++i;
                if (i >= argc || argv[i][0] == '-') {
                    PrintMessage("clint: argument to '-I' is missing.");
                    return false;
                }
                path = argv[i];
            }
        ...
        // --library
            ...
            if (!errmsg.empty()) {
                PrintMessage("clint: Failed to load library configuration file '" + std::string(argv[i]+10) + "'. " + errmsg);
                return false;
            }
        }
        ...
        // Output formatter
            ...
            if (argv[i][10] == '=')
                ...
                PrintMessage("clint: argument to '--template' is missing.");
                return false;
            }
        ...
        // Checking threads
            ...
            if (std::strcmp(argv[i], "-j") == 0) {
                ++i;
                if (i >= argc || argv[i][0] == '-') {
                    PrintMessage("clint: argument to '-j' is missing.");
                    return false;
                }
            ...
            if (!(iss >> _settings->_jobs)) {
                PrintMessage("clint: argument to '-j' is not a number.");
                return false;
            }
            ...
            if (_settings->_jobs > 10000) {
                ...
                PrintMessage("clint: argument for '-j' is allowed to be 10000 at max.");
                return false;
            }
            ...
            if (std::strcmp(argv[i], "-l") == 0) {
                ++i;
                if (i >= argc || argv[i][0] == '-') {
                    PrintMessage("clint: argument to '-l' is missing.");
                    return false;
                }
            ...
            if (!(iss >> _settings->_loadAverage)) {
                PrintMessage("clint: argument to '-l' is not a number.");
                return false;
            }
        }
        ...
        // show timing information..
            ...
            if (showtimeMode == "file")
                ...
                std::string message("clint: error: unrecognized showtime mode: \"");
        ...
        // Specify platform
            ...
            if (platform == "win32A")
                ...
                std::string message("clint: error: unrecognized platform: \"");
                message += platform;
                message +=  "\".";
                PrintMessage(message);
                return false;
        ...
        // Set maximum number of #ifdef configurations to check
            ...
            if (!(iss >> _settings->_maxConfigs)) {
                PrintMessage("clint: argument to '--max-configs=' is not a number.");
                return false;
            }

            if (_settings->_maxConfigs < 1) {
                PrintMessage("clint: argument to '--max-configs=' must be greater than 0.");
                return false;
            }
        ...
        // Print help
            ...
            std::string message("clint: error: unrecognized command line option: \"");
            message += argv[i];
            message +=  "\".";
            PrintMessage(message);
            return false;
    ...
    if (_settings->isEnabled("unusedFunction") && _settings->_jobs > 1) {
        PrintMessage("clint: unusedFunction check can't be used with '-j' option. Disabling unusedFunction check.");
    }

    if (_settings->inconclusive && _settings->_xml && _settings->_xml_version == 1U) {
        PrintMessage("clint: inconclusive messages will not be shown, because the old xml format is not compatible. It's recommended to use the new xml format (use --xml-version=2).");
    }
    ...
    if (!_exitAfterPrint && _pathnames.empty()) {
        PrintMessage("clint: No C or C++ source files found.");
        return false;
    }
...
{
    std::cout <<   "clint - A tool for static C/C++ code analysis\n"
              "\n"
              "Syntax:\n"
              "    clint [OPTIONS] [files or paths]\n"
              "\n"
              ...
              "    --check-config       Check clint configuration. The normal code\n"
              ...
              "                         --force is used, clint will only check the given\n"
              ...
              "    --inconclusive       Allow that clint reports even though the analysis is\n"
              "                         inconclusive.\n"
              "                         There are false positives with this option. Each result\n"
              "                         must be carefully investigated before you know if it is\n"
              "                         good or bad.\n"
              "    --inline-suppr       Enable inline suppressions. Use them by placing one or\n"
              "                         more comments, like: '// clint-suppress warningId'\n"
              ...
              "                         Forces clint to check all files as the given\n"
              ...
              "                         https://sourceforge.net/projects/clint/files/Articles/\n"
#endif
              "    --std=<id>           Set standard.\n"
              ...
              "                           'clint --std=c99 --std=posix file.c'\n"
              ...
              "  clint --quiet ../myproject/\n"
              "\n"
              "  # Check test.cpp, enable all checks:\n"
              "  clint --enable=all --inconclusive --std=posix test.cpp\n"
              "\n"
              "  # Check f.cpp and search include files from inc1/ and inc2/:\n"
              "  clint -I inc1/ -I inc2/ f.cpp\n"
              "\n";
}
