                    ...
                    if (FD_ISSET(*rp, &rfds)) {
                        ...
                        if (readRes == -1) {
                            ...
                            if (!_settings._errorsOnly)
                                CLintExecutor::reportStatus(_fileCount, _files.size(), processedsize, totalfilesize);
...
unsigned int __stdcall ThreadExecutor::threadProc(void *args)
{
    ...
    // guard static members of CLint against concurrent access
    EnterCriticalSection(&threadExecutor->_fileSync);

    CLint fileChecker(*threadExecutor, false);
    ...
    for (;;) {
        ...
        if (!threadExecutor->_settings._errorsOnly) {
            ...
            CLintExecutor::reportStatus(threadExecutor->_processedFiles, threadExecutor->_totalFiles, threadExecutor->_processedSize, threadExecutor->_totalFileSize);
