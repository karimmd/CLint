...
bool CLintExecutor::parseFromArgs(CLint *clint, int argc, const char* const argv[])
{
    Settings& settings = clint->settings();
    CmdLineParser parser(&settings);
    const bool success = parser.ParseFromArgs(argc, argv);

    if (success) {
        if (parser.GetShowVersion() && !parser.GetShowErrorMessages()) {
            const char * extraVersion = clint->extraVersion();
            if (*extraVersion != 0)
                std::cout << "Clint " << clint->version() << " ("
                          << extraVersion << ')' << std::endl;
            else
                std::cout << "Clint " << clint->version() << std::endl;
        }

        if (parser.GetShowErrorMessages()) {
            errorlist = true;
            std::cout << ErrorLogger::ErrorMessage::getXMLHeader(settings._xml_version);
            clint->getErrorMessages();
        ...
        for (iter = settings._includePaths.begin();
            ...
            if (FileLister::isDirectory(path))
                ...
                if (!settings._errorsOnly)
                    std::cout << "clint: warning: Couldn't find path given by -I '" << path << '\'' << std::endl;
    ...
    if (!_files.empty()) {
        ...
        if (warn) {
            std::cout << "clint: filename exclusion does not apply to header (.h and .hpp) files." << std::endl;
            std::cout << "clint: Please use --suppress for ignoring results from the header files." << std::endl;
        }
        ...
        std::cout << "clint: error: could not find or open any of the paths given." << std::endl;
        return false;
    }
    ...
    if (!_files.empty()) {
        ...
        std::cout << "clint: error: no files to check - all paths ignored." << std::endl;
        return false;
    }
}

int CLintExecutor::check(int argc, const char* const argv[])
{
    Preprocessor::missingIncludeFlag = false;
    Preprocessor::missingSystemIncludeFlag = false;

    CLint cLint(*this, true);

    Settings& settings = cLint.settings();
    _settings = &settings;

    if (!parseFromArgs(&cLint, argc, argv)) {
        return EXIT_FAILURE;
    }
    ...
    if (cLint.settings().exceptionHandling) {
        return check_wrapper(cLint, argc, argv);
    } else {
        return check_internal(cLint, argc, argv);
    }
}
 ...
 */
static void ClintSignalHandler(int signo, siginfo_t * info, void * /*context*/)
{
    const char * const signame = signal_name(signo);
    const char * const sigtext = strsignal(signo);
    bool bPrintCallstack=true;
    FILE* f=CLintExecutor::getExceptionOutput()=="stderr" ? stderr : stdout;
    fputs("Internal error: clint received signal ", f);
    fputs(signame, f);
    fputs(", ", f);
    fputs(sigtext, f);
    switch (signo) {
    ...
    if (bPrintCallstack) {
        ...
        fputs("\nPlease report this to the clint developers!\n", f);
    }
    abort();
}
#endif
 ...
 *
 */
int CLintExecutor::check_wrapper(CLint& clint, int argc, const char* const argv[])
{
#ifdef USE_WINDOWS_SEH
    FILE *f = stdout;
    __try {
        ...
        fputs("Please report this to the clint developers!\n", f);
        return -1;
    }
    ...
    act.sa_sigaction=ClintSignalHandler;
 ...
 * */
int CLintExecutor::check_internal(CLint& clint, int /*argc*/, const char* const argv[])
{
    Settings& settings = clint.settings();
    _settings = &settings;
    ...
    if (!std || !posix) {
        ...
        const std::string msg("Failed to load " + std::string(!std ? "std.cfg" : "posix.cfg") + ". Your clint installation is broken, please re-install.");
#ifdef CFGDIR
        const std::string details("The Clint binary was compiled with CFGDIR set to \"" +
        ...
        const std::string details("The Clint binary was compiled without CFGDIR set. Either the "
    ...
    if (settings._jobs == 1) {
        ...
        for (std::map<std::string, std::size_t>::const_iterator i = _files.begin(); i != _files.end(); ++i) {
            if (!_settings->library.markupFile(i->first)
                ...
                returnValue += clint.check(i->first);
    ...
    if (!settings.checkConfiguration) {
        clint.tooManyConfigsError("",0U);

        if (settings.isEnabled("missingInclude") && (Preprocessor::missingIncludeFlag || Preprocessor::missingSystemIncludeFlag)) {
            const std::list<ErrorLogger::ErrorMessage::FileLocation> callStack;
            ErrorLogger::ErrorMessage msg(callStack,
                                          Severity::information,
                                          "Clint cannot find all the include files (use --check-config for details)\n"
                                          "Clint cannot find all the include files. Clint can check the code without the "
                                          "include files found. But the results will probably be more accurate if all the include "
                                          "files are found. Please check your project's include directories and add all of them "
                                          "as include directories for Clint. To see what files Clint cannot find use "
                                          "--check-config.",
...
void CLintExecutor::reportErr(const std::string &errmsg)
{
...
void CLintExecutor::reportOut(const std::string &outmsg)
{
    std::cout << outmsg << std::endl;
}

void CLintExecutor::reportProgress(const std::string &filename, const char stage[], const std::size_t value)
{
    (void)filename;
...
void CLintExecutor::reportInfo(const ErrorLogger::ErrorMessage &msg)
{
    reportErr(msg);
}

void CLintExecutor::reportStatus(std::size_t fileindex, std::size_t filecount, std::size_t sizedone, std::size_t sizetotal)
{
    if (filecount > 1) {
...
void CLintExecutor::reportErr(const ErrorLogger::ErrorMessage &msg)
{
    if (errorlist) {
...
void CLintExecutor::setExceptionOutput(const std::string& fn)
{
    exceptionOutput=fn;
}

const std::string& CLintExecutor::getExceptionOutput()
{
    return exceptionOutput;
}

std::string CLintExecutor::exceptionOutput;
